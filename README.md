...
아이콘 방향 제안:
- Duck + Dock 언어유희 기반 아이콘
- 드라이브 + 도킹 심볼
- `M` / `D` 모노그램 기반 아이콘

현재 저장소에는 duck 컨셉의 1차 아이콘 시안이 포함되어 있습니다:
...
- Duck + Dock wordplay icon
- drive + docking symbol
- `M` / `D` monogram concept

A first duck-themed branding draft is now included in the repository:
- `assets/branding/mountdock-icon.svg`
- `assets/branding/MountDock_branding.md`
